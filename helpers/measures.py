...
def get_jpeg2000_size(img):
    """
    Get compressed size of JPEG2000 from raw pixel data, by compressing it
    ...
    Return:
        - s (int) : size of compressed file (number of bytes)
    """
    fname = f"{str(uuid.uuid4())}.jp2"
    io.imsave(fname, img)
    s = os.path.getsize(fname)
    os.remove(fname)
    return s

def get_jpeg_size(img, q):
    """
    Get compressed size of JPEG2000 from raw pixel data, by compressing it
    ...
    Return:
        - s (int) : size of compressed file (number of bytes)
    """
    fname = f"{str(uuid.uuid4())}.jpeg"
    io.imsave(fname, img, quality=q)
    s = os.path.getsize(fname)
    os.remove(fname)
    return s
