...
def page():
    ...
    df_train = df_train.drop('$GO-\ell^0$', axis=1)\
    ...
    df_test = df_test.drop('$GO-\ell^0$', axis=1)\
                    .drop('$GO-\ell^1$', axis=1)\
                    .drop('$GO-\ell^2$', axis=1)\
                    .drop('$GO-\ell^H$', axis=1)\
                    .drop('$DoG-\ell^0$', axis=1)\
    ...
    with col1:
        st.markdown('''Predict PNG Size using sparsity measurements. Lasso Regression, using $\lambda=1$.
                    This time, using Cardinality indicator and Hopkins Statistic ''')

        with open('models/second_predictor_png.pkl', 'rb') as f:
            model = pickle.load(f)
